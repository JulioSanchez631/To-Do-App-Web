...
// Importacion de archivo tipado estandar
...
const contenedorSeccionTareas = document.querySelector(".contenedor-tareas") as HTMLDivElement;
// ----------------------------------------------------------------------------------------------
...
const eventos = () => {
  ...
  btnEliminarTareas.forEach(item => {
    item.addEventListener("click",() => {
      ...
      if(typeof valorIdTarea == 'string'){
        const valorIdTareaEntero : number = parseInt(valorIdTarea,10);
  ...
  btnsAbrirModal.forEach(item => {
    item.addEventListener('click',() => {
      ...
      if(valorItem){
        const texto = document.getElementById('message-text') as HTMLInputElement;
        console.log(texto);
        const valorIdItem : number = parseInt(valorItem?.innerHTML);
  ...
  btnGuardarTarea?.addEventListener('click',() => {
    const textoHTML = document.getElementById('message-text') as HTMLInputElement;
  ...
  const inputFiltrado = document.getElementById('input-filtros') as HTMLSelectElement;
  // const btnActivadorFiltrado = document.getElementById('btn-filtrado') as HTMLButtonElement;
  const formFiltrado = document.getElementById('form-tareas-filtro') as HTMLFormElement;

  const tareasDOM = document.querySelectorAll('.item-tarea');
  const tareasTotales = obtenerTareasLocal()
  let idSeleccion : number[] = [];
  ...
  formFiltrado.addEventListener('submit',(e) => {
    ...
    if(inputFiltrado.value == 'Todas'){
      ...
      tareasTotales.forEach((item : Todo) => {
...
export const RenderizarTareasHTML = () => {
  ...
  if(tareasTotales){
    const contenedorListadoTareas = document.querySelector(".contenedor-listado-tareas") as HTMLDivElement;
...
const activadorBtnEliminarTarea = (id : number) => {
  eliminarTarea(id);
}

const activadorBtnCompletarTarea = (id : number) => {
  completarTarea(id);
}
